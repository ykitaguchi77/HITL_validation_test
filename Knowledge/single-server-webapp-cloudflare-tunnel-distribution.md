...
## 複数作業者の同時利用が成立する条件 — 追記 (2026-09-15)
- 出典: Experimental_record/20260915.md(追記7)
- 「同時に複数人がログインしたら?」に対し**サーバをステートレスに設計しておけば問題なし**と回答できた。成立条件は次の4点:
  1. **記録先を人×セッション開始時刻で分離**(`outputs/results/{名前}_{セッション}_a{試行}_{時刻}/`)し、追記先ファイルが
     人ごとに違うこと(同一ファイルへの並行 append を作らない)。
  2. **進捗(どこまで終えたか)をサーバに持たずクライアント localStorage(名前ごとキー分離)に置く**こと。副作用として
     同一人物が2台で開くと進捗がずれるので「同じPC・同じブラウザで」の運用注記が必要(記録自体は壊れない)。
  3. **推論は起動時に全モデルを GPU ウォームアップ**(小モデル×8=約50MB)し1回 0.1秒程度に抑える。uvicorn 1プロセスでも
     スレッドプールで並列化され、3人同時で体感遅延なし。加えて**推論待ちを作業時間から除外**していれば混雑が計測を歪めない。
  4. 名前の取り違えは共有PWでは技術的に防げない → ガイドの注意書き、または Cloudflare Access(メールPIN)で人を識別。
